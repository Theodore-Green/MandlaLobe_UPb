...
## --- Sample Properties. Keep this sample order or change all the other files to match another order 
    ...
    smpl.Name = ("DEC13-30", "RBAB", "RBAG", "RBBQ", "RBBL", "RBBI", "RBBM", "RBBR", "RBBJ", "RBBS", "RBAW", "RBX", "RBBH", "RBBF", "RBB2", "RBAY", "RBAO", "RBAN", "RBP", "RBO", "RBE", "RBF", "DEC13-08", "RBG")
    smpl.Height .= [5, 60, 80, 115, 137, 145, 173, 195, 240, 275, 290, 300, 330, 355, 385, 395, 420, 434, 474, 488, 520, 530, 538, 590]
    smpl.Height_sigma .= zeros(nSamples)
    smpl.Age_Sidedness .= zeros(nSamples) # Sidedness (zeros by default: geochron constraints are two-sided). Use -1 for a maximum age and +1 for a minimum age, 0 for two-sided
    smpl.Path = "/Users/tg5176/Documents/GitHub/MandlaLobe_UPb/WesternGhatsData/DeccanUPbData" # Where are the data files?
    smpl.inputSigmaLevel = 2; # i.e., are the data files 1-sigma or 2-sigma. Integer.
    smpl.Age_Unit = "Ma"
    smpl.Height_Unit = "km^3*1000"
...
## --- Bootstrap pre-eruptive distribution - - - - - - - - - - - - - - - - - - -
    ...
    #    framestyle=:box
    #)
    #savefig(h, joinpath(smpl.Path,"BootstrappedDistribution_WesternGhats.pdf"))
    #display(h)
## --- Estimate the eruption age distributions for each sample  - - - - - - - -
    ...
    distBurnin = floor(Int,distSteps/5) # Number to discard? Should it be this (2019 version) or distSteps÷100 (current Chron.jl example)
    ...
    # dist = TriangularDistribution
     dist = [2.0,1.0,0.0] # Triangular distribution used in Schoene et al. 2019 preferred interpretation
...
## --- Plot stratigraphic model - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    savefig(hdl,joinpath(smpl.Path, "AgeDepthModel_WesternGhats_Schoene2019.pdf"))
    display(hdl)
    ...
    #savefig(hdl, "AgeDepthModel_WesternGhats_withstratconstraints.pdf")
...
## --- Parameters for age distribution of each sample from stratigraphic model outputs (age distribution at a given height) 
    ...
    CSV.write(joinpath(smpl.Path, "distparameters_Schoene2019.csv"), StratigraphicDistributionParameters)
